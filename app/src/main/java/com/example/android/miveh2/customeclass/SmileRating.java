...
//    private Path mEyePathRight = new Path();
    ...
    private void getSmiley(Smileys smileys, float fraction, float divisions, float fromRange,
        ...
        if (fraction > 0.75f) {
            fraction -= 0.75f;
            fraction *= 4;
            findNearestSmile(fraction, GOOD, EXCELLENT);
            mBackgroundPaint.setColor(mNormalColor);
            transformSmile(trans, fraction, smilePath,
                    smileys.getSmile(GOOD), smileys.getSmile(EXCELLENT), mFloatEvaluator);
            createEyeLocation(smileys, divisions, fraction, actualTranslation, EXCELLENT, smilePath, smilePath, centerY);
    ...
    public interface OnSmileySelectionListener {
         ...
         *                   {@link #OKAY},{@link #GOOD},{@link #EXCELLENT})
